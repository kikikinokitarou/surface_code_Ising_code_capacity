...
def gene_costfunc():
    ...
    #2つのスタビライザーに挟まれたビット
    H_cost+=spin_to_binary( -binary_to_spin(ER_pure[0][0])*binary_to_spin(x[0]) )*spin_to_binary( -binary_to_spin(ER_pure[1][0])*binary_to_spin(z[0]) )
    H_cost+=spin_to_binary(- binary_to_spin(ER_pure[0][ls-1])*binary_to_spin(x[ls-2]))*spin_to_binary(-binary_to_spin(ER_pure[1][ls-1])*binary_to_spin(z[ls-1]) )
    H_cost+=spin_to_binary(-binary_to_spin(ER_pure[0][ls*(ls-1)])*binary_to_spin(x[(ls-1)**2]))*spin_to_binary(-binary_to_spin(ER_pure[1][ls*(ls-1)])*binary_to_spin(z[ls*(ls-2)]))
    H_cost+=spin_to_binary(- binary_to_spin(ER_pure[0][ls**2-1])*binary_to_spin(x[ls*(ls-1)-1]))*spin_to_binary(- binary_to_spin(ER_pure[1][ls**2-1])*binary_to_spin(z[ls*(ls-1)-1]))

    #3つのスタビライザーに囲まれた量子ビット
    for i in range(1,ls-1):
        H_cost+=spin_to_binary(-binary_to_spin(ER_pure[0][i])*binary_to_spin(x[i-i//ls-1])*binary_to_spin(x[i-i//ls]))*spin_to_binary(-binary_to_spin(ER_pure[1][i])*binary_to_spin(z[i]))
        H_cost+=spin_to_binary(-binary_to_spin(ER_pure[0][ls*i])*binary_to_spin(x[(ls-1)*i]))*spin_to_binary(-binary_to_spin(ER_pure[1][ls*i])*binary_to_spin(z[ls*i-ls])*binary_to_spin(z[ls*i]))
        H_cost+=spin_to_binary(-binary_to_spin(ER_pure[0][ls*i+(ls-1)])*binary_to_spin(x[ls*i+(ls-1)-(i+1)]))*spin_to_binary(-binary_to_spin(ER_pure[1][ls*i+(ls-1)])*binary_to_spin(z[ls*i+(ls-1)-ls])*binary_to_spin(z[ls*i+(ls-1)]))
        H_cost+=spin_to_binary(-binary_to_spin(ER_pure[0][ls*(ls-1)+i])*binary_to_spin(x[ls*(ls-1)+i-ls])*binary_to_spin(x[ls*(ls-1)+i-ls+1]))*spin_to_binary(-binary_to_spin(ER_pure[1][ls*(ls-1)+i])*binary_to_spin(z[ls*(ls-1)+i-ls]))
        
        
    #4つのスタビライザーに囲まれた量子ビット
    for i in range(1,ls-1):
        for j in range(1,ls-1):
            H_cost+=spin_to_binary(-binary_to_spin(ER_pure[0][ls*i+j])*binary_to_spin(x[(ls-1)*i+j-1])*binary_to_spin(x[(ls-1)*i+j]))*spin_to_binary(-binary_to_spin(ER_pure[1][ls*i+j])*binary_to_spin(z[ls*(i-1)+j])*binary_to_spin(z[ls*i+j]))

            
            
    for i in range(ls-1):
        for j in range(ls-1):
            H_cost+=spin_to_binary(-binary_to_spin(ER_pure[0][ls**2+(ls-1)*i+j])*binary_to_spin(x[(ls-1)*i+j])*binary_to_spin(x[(ls-1)*(i+1)+j]))*spin_to_binary(-binary_to_spin(ER_pure[0][ls**2+(ls-1)*i+j])*binary_to_spin(z[ls*i+j])*binary_to_spin(z[ls*i+j+1]))
    return H_cost
...
def solve(H):
    ...
    sampler=oj.SASampler(num_reads=1,num_sweeps=1000)
